...
<style>
    ...
    .order-row:hover {
        background: rgba(56, 189, 248, 0.08); /* Blue Tint on Hover */
...
<div class="d-flex justify-content-between align-items-center mb-4 mt-2">
    <div>
        ...
        <p class="text-secondary small mb-0">Manage service requests efficiently.</p>
    </div>
    <div class="d-flex gap-2">
        <button onclick="location.reload()" class="btn btn-outline-light d-flex align-items-center gap-2">
...
<div class="row g-3 mb-4">
    <div class="col-md-4">
        <div class="glass-card p-3 d-flex align-items-center justify-content-between">
            <div>
                <small class="text-secondary text-uppercase fw-bold">Total Orders</small>
                <h4 class="text-white mb-0 fw-bold"><?= $total_o ?></h4>
            </div>
            <div class="p-3 rounded-circle" style="background: rgba(56, 189, 248, 0.1); color: #38bdf8;"><i class="fas fa-box"></i></div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="glass-card p-3 d-flex align-items-center justify-content-between">
            <div>
                <small class="text-secondary text-uppercase fw-bold">Pending</small>
                <h4 class="text-warning mb-0 fw-bold"><?= $pending_o ?></h4>
            </div>
            <div class="p-3 rounded-circle" style="background: rgba(245, 158, 11, 0.1); color: #fbbf24;"><i class="fas fa-clock"></i></div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="glass-card p-3 d-flex align-items-center justify-content-between">
            <div>
                <small class="text-secondary text-uppercase fw-bold">Completed Revenue</small>
                <h4 class="text-success mb-0 fw-bold">SAR <?= number_format($revenue_o) ?></h4>
            </div>
        ...
        <form method="GET" class="row g-3 align-items-center">
            <div class="col-md-5">
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control border-start-0 ps-0" placeholder="Search by Order ID or Customer Name..." value="<?= $_GET['search'] ?? '' ?>">
                </div>
            </div>
            <div class="col-md-3">
                ...
                <tr class="order-row" onclick="window.location='order_details.php?id=<?= $row['id'] ?>'" style="cursor: pointer;">
                    ...
                    <td class="ps-4">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <small class="text-secondary d-block"><?= $row['schedule_time'] ?></small>
                                <span class="text-white fw-bold">#ORD-<?= str_pad($row['id'], 4, '0', STR_PAD_LEFT) ?></span>
                            </div>
                        </div>
                    </td>

                    <td>
                        <span class="text-light small">Service Request</span>
                    </td>

                    <td>
                        <div class="d-flex align-items-center">
                            <?php $c_img = !empty($row['c_image']) ? "../api/uploads/".$row['c_image'] : "https://ui-avatars.com/api/?name=".$row['c_name']."&background=0f172a&color=fff"; ?>
                            <img src="<?= $c_img ?>" class="rounded-circle border border-secondary me-2" width="30" height="30">
                            <span class="text-white fw-bold small"><?= $row['c_name'] ?></span>
                        </div>
                    </td>
                    ...
                    <td>
                        <?php if($row['p_name']): ?>
                            <div class="d-flex align-items-center">
                                <?php $p_img = !empty($row['p_image']) ? "../api/uploads/".$row['p_image'] : "https://ui-avatars.com/api/?name=".$row['p_name']."&background=38bdf8&color=fff"; ?>
                                <img src="<?= $p_img ?>" class="rounded-circle border border-primary me-2" width="30" height="30">
                                <span class="text-info small"><?= $row['p_name'] ?></span>
                            </div>
                        <?php else: ?>
                            <span class="badge bg-secondary text-white small bg-opacity-25 border border-secondary">Unassigned</span>
                        <?php endif; ?>
                    </td>
